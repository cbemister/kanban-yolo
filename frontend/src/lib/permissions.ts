...
export async function checkBoardPermissionWithBoard(boardId: string, requiredRole: "OWNER" | "EDITOR" | "VIEWER") {
  const userId = await getAuthenticatedUserId();
  if (!userId) return { authorized: false as const, response: NextResponse.json({ error: "Unauthorized" }, { status: 401 }) };
  ...
  if (board.userId === userId) {
    return { authorized: true as const, userId, role: "OWNER" as const, board };
  }
  ...
  return { authorized: true as const, userId, role: member.role, board };
}
